...
```js
self.onkeydown=e=>![e.keyCode-43?console.log(e.key):alert`Press the correct key.`];
```
# Major Redesigns
...
Grouping commonly repeated code throughout a script may drastically reduce its length. Functions are the most flexible method to group statements, although execution speed dips when called often. As mentioned before, they can be modified to call themselves, and even replace finite loops.
```js
