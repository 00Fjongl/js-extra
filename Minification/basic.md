...
a=a=='c'?'b':'c';
```
Bitwise operators `&` and `|` can be used similarly to `&&` and `||`, under much stricter circumstances. Unlike logical operators, functions and assignments will run regardless. Avoid combining operators (and sometimes numbers other than `1` or `0`), if they are used as consistent booleans. Bitwise also ignores decimals.
```js
if(a>3&&a<6)a=0;
a>3&a<6&&(a=0);
//even better
