...
```
Brackets are useful for working with strings and 64-bit integers. They can be useful when a number, `undefined`, or `null` is involved, but still function as arrays if not concatenated.
```js
